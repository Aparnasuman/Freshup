...
class Payment extends CI_Controller {
	...
	public function index(){
		$data['stripeDetails'] = $this->db->get_where('PaymentSetting',array('id' => 1))->row_array();
		$data['cashDetails'] = $this->db->get_where('PaymentSetting',array('id' => 2))->row_array();
		$data['paypalDetails'] = $this->db->get_where('PaymentSetting',array('id' => 3))->row_array();
		$admin_details = $this->session->userdata('admin_details');
		$data['active'] = 'payment';
		$data['title']  = 'Payment Modes';
		$data['admin'] = $this->Common_model->get_data_by_id('admin','id',$admin_details['admin_id']);
		$this->load->view('admin/includes/header',$data);
		$this->load->view('admin/paymentSetting/managePayment');
		$this->load->view('admin/includes/footer');
	}
	
	public function stripeStatusChange(){
		$checkStripeStatus = $this->db->get_where('PaymentSetting',array('id' => 1))->row_array();
		if($checkStripeStatus['status'] == '1'){
			$details['status'] = '0';
		}
		else{
			$details['status'] = '1';
		}
		$update = $this->Common_model->update('PaymentSetting',$details,'id',1);
		if($details['status'] == '1'){
			echo 1;
		}
		else{
	...
	public function paypalStatusChange(){
		$checkStripeStatus = $this->db->get_where('PaymentSetting',array('id' => 3))->row_array();
		if($checkStripeStatus['status'] == '1'){
			$details['status'] = '0';
		}
		else{
		...
		$update = $this->Common_model->update('PaymentSetting',$details,'id',3);
		if($details['status'] == '1'){
			echo 1;
		}
		else{
	...
	public function cashStatusChange(){
		$checkStripeStatus = $this->db->get_where('PaymentSetting',array('id' => 2))->row_array();
		if($checkStripeStatus['status'] == '1'){
			$details['status'] = '0';
		}
		else{
		...
		$update = $this->Common_model->update('PaymentSetting',$details,'id',2);
		if($details['status'] == '1'){
			echo 1;
		}
		else{
